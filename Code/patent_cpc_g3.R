...
for (year in years){
  ...
  data<-patent_cpc_data%>%filter(year_issue>=year-5 & year_issue<=year-1)%>%
    group_by(permno,cpc_g3)%>%
    mutate(number=n())%>%
    select(cpc_g3,permno,number)%>%ungroup()%>%
    distinct(permno,cpc_g3,.keep_all = T)
  
  
  permno<-data%>%distinct(permno)%>%arrange(permno)
  cpc_g3<-data%>%distinct(cpc_g3)%>%filter(cpc_g3!="NA")
  
  
  
  data_year<-merge(cpc_g3,permno,all=T)%>%arrange(permno,cpc_g3)%>%left_join(data,by=c("permno","cpc_g3"))
  
  for (i in 1:nrow(data_year)){
    if (is.na(data_year$number[i])) {data_year$number[i]<-0}
  }
  
  data_year_reshape<-reshape(data_year,direction = "wide",idvar = "cpc_g3",timevar="permno")
  ...
  filename<-paste("cor_g3_",year,".RData",sep="")
